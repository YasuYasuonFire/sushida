...
1. `npm ci` → `npm run lint` → `npm test -- --run` → `npm run build`
2. `npm i -g vercel`（未インストールの場合）
3. `vercel login` でログインし、`vercel link` でプロジェクトを紐付け
4. `vercel` でプレビューを確認し、問題なければ `vercel --prod`

詳細な手順や補足は **[Vercel へのデプロイ手順](docs/deployment-vercel.md)** を参照してください。
